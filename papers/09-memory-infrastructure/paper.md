...
A technical question remains open. Kenny (2026, arXiv:2511.20321) argues that EFE as conventionally formulated is mathematically ill-defined — the statistical-independence approximations used to decompose epistemic and pragmatic value imply a mutual-information quantity that evaluates to zero, making the objective formally incoherent when examined closely. His proposed resolution is to decouple active inference from EFE entirely and use a unified KL-divergence objective instead. The hybrid-architecture proposals cited above adopt EFE without engaging this critique. The "Missing Reward" paper temporally precedes Kenny's January 2026 publication, which excuses the specific citation but not the general issue: similar concerns about EFE's mathematical foundations had been raised previously in the active-inference literature, and the paper does not engage them.

Friston's own most recent foundational work (Friston, Ramstead & Sakthivadivel 2026, arXiv:2406.11630v5, *Proceedings A*) sharpens the position further. Friston now explicitly characterizes the FEP as licensing a "useful explanatory fiction" rather than an ontological claim, writing that "the fact that a physical object conforms to the FEP does not necessarily imply that this object performs inference in the literal sense; rather, it is a useful explanatory fiction which replaces the 'explicit' dynamics of the object with an 'implicit' flow on free energy gradients." This is a significant softening: structural correspondence under the FEP does not commit one to a claim that the system is literally computing free-energy gradients. The interpretive stance and the substrate-mechanics are explicitly decoupled.

The implicit-vs-explicit energy distinction, which the *Proceedings A* paper makes load-bearing, is precisely what allows our scaffold-as-RGM framing to survive Kenny's critique. We claim structural correspondence (hierarchical generative modeling under sparse coupling and Markov-blanket partitioning) without claiming dynamic equivalence (that the scaffold is performing variational inference at runtime). Under Friston's 2026 framing, structural correspondence is the licensing condition for the FEP description as an explanatory fiction; it does not require Kenny's EFE objective to be coherent because it does not commit to a specific objective at all. The scaffold's compile cycle is observed to behave as a selection mechanism that promotes stable, low-energy candidates in a sense one can describe in implicit-energy terms. Whether the implicit description maps to a coherent *explicit* energy function (the question Kenny presses against EFE) is left open.

Our own use of EFE-derived epistemic/pragmatic decomposition at the retrieval layer (see `vault_search.py --efe` mode, shipped from prior convergent-evidence work) adopts the *decomposition* as a retrieval-ranking heuristic without committing to EFE's status as the foundational objective. The decomposition is operationally motivated — it improves ranking on tasks where retrieval relevance and information-gain diverge — and is robust under both the hybrid-architecture proposals' framework (which retains EFE as foundational) and Kenny's decoupling framework (which replaces EFE with KL-divergence at the foundational layer while preserving the per-step decomposition as a useful heuristic). The position is therefore double-defensible: we cite the hybrid proposals to establish that LLM+AIF is a named research direction and that our scaffold can be described in compatible terms, while citing Kenny to establish that we have engaged the strongest existing objection to EFE without committing to its formal status.

What remains genuinely open is whether the structural correspondence between the scaffold and an RGM is empirically tighter than the convergent description suggests. The §5.10 and §5.12 evidence — Khushiyant phase transition, Bressloff Kuramoto coupling, Tero conductivity rule, Adamatzky three-memory taxonomy — is convergent at the structural level. None of those convergences require committing to a specific dynamic objective. The position §5.11.1 takes is therefore conservative-by-design: structure is the load-bearing claim, dynamics is left open, and the EFE controversy is engaged rather than sidestepped.

#### 5.11.2 Cautionary Contrast — Borrowed Prestige Without Formal Discharge

We distinguish our framework from proposals that adopt loaded terminology (autopoiesis, consciousness, integration) without formally discharging the burdens those terms carry. A recent example: Connor & Defant (2026, arXiv:2601.04501) claim a "formally provable autopoietic primitive" based on a proof structure that establishes stochastic-process convergence (iterated-random-functions theory → stationary distribution) rather than organizational closure. Convergence to a stationary distribution is a property Markov chains and most ergodic processes have; it is a vastly weaker claim than the three Maturana-Varela autopoietic criteria (organizational closure, operational openness, self-production of components). None of the three criteria are formally discharged in the paper's construction, no primary Maturana-Varela sources are cited, and the paper does not engage Varela's own later anti-formalization position (cf. Thompson 2007, Di Paolo 2005).

We note this conflation to sharpen our own claims by contrast. Our scaffold framework does not claim formally provable autopoiesis. It claims *bounded* autopoiesis: full closure at the informational level (the compile cycle is closed), partial closure at the operational level (human approval remains in the loop), and open-ended closure at the developmental level (the scaffold's own rules are amended by the compile cycle it enables). These are weaker and more defensible claims. The Connor-Defant contrast is useful precisely because it shows what "formally provable autopoiesis" looks like when the label is adopted without the formal content. Our three-level bounded claim is not formally provable in the Connor-Defant sense; it does not pretend to be.
